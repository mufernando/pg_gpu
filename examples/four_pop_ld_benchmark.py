#!/usr/bin/env python
"""
Benchmark: pg_gpu four-population LD statistics vs moments.

Validates that pg_gpu produces the same LD statistics (DD, Dz, pi2)
as moments for a four-population model, and compares wall-clock time.

Usage:
    pixi run -e moments python examples/four_pop_ld_benchmark.py
"""
...
def simulate_data():
    """Simulate a 4-population tree and write VCF + pop file."""
    CACHE_DIR.mkdir(exist_ok=True)
    vcf_path = CACHE_DIR / "four_pop_benchmark.vcf"
    pop_path = CACHE_DIR / "four_pop_benchmark_pops.txt"
    ...
    demography.add_population(name="pop3", initial_size=1000)
...
def run_moments(vcf_path, pop_path, use_cache=True):
    ...
    cache_file = CACHE_DIR / "moments_ld_4pop.pkl"
...
def plot_results(mom, gpu, labels, stat_names, t_moments, t_pg):
    ...
    ax.set_title("Four-population LD statistics")
    ...
    fig.suptitle("pg_gpu vs moments: four-population LD (DD, Dz, pi2)\n"
                 f"4-pop split model, {len(stat_names)} statistics x {n_bins} distance bins",
                 fontsize=13, y=1.02)
    plt.tight_layout()
    fig.savefig("examples/four_pop_ld_benchmark.png", dpi=150, bbox_inches="tight")
    print("\nFigure saved to examples/four_pop_ld_benchmark.png")


def main():
    print("Four-population LD benchmark: pg_gpu vs moments")
    print("=" * 55)
